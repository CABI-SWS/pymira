...
class SpatialGraph(amiramesh.AmiraMesh):
    ...
    **Applications:**
    ...
    def get_domain(self):
        return arr(self.edge_spatial_extent())
    ...
    def point_scalars_to_node_scalars(self,mode='max',name=None,func=['max']):
        ...
        # If OOM, try an alternate approach...
        ...
        if callable(func):
            # Only pass the 1D node_mean array (no need for masked_data)
            node_scalar = func(node_mean)
            # If you want None where there was no data:
            node_scalar = np.where(node_count == 0, None, node_scalar).astype(data.dtype)
            return node_scalar
        elif isinstance(func, list) and all(callable(f) for f in func):
            res = []
            for f in func:
                node_scalar = f(node_mean)
                node_scalar = np.where(node_count == 0, None, node_scalar).astype(data.dtype)
                res.append(node_scalar)
            return res
        else:
